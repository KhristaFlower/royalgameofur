...
var events = {
  ...
  lobby: {
    ...
    challenge: {
       ...
       */
      exists: function exists(opponentName) {
  ...
  game: {
    ...
    activity: function activity(gameData) {
      ...
      if (currentGame !== null && currentGame.id === gameData.game.id) {
        ...
        var copyProps = Object.keys(game);
        ...
        if ('move' in delta) {
          moveToken(currentGame.id, delta.move.pid, delta.move.t, delta.move.m).then(function () {
        ...
        for (var _i = 0; _i < lobbyGameList.length; _i++) {
          // Search for the existing game item.
          if (lobbyGameList[_i].gameId === gameData.game.id) {
            // We found the game entry, hydrate a game object so we can use helper functions.
            lobbyGameList[_i].currentPlayer.id = game.getCurrentPlayer().pid;
            lobbyGameList[_i].currentPlayer.name = game.getCurrentPlayer().name;
            lobbyGameList[_i].turn.number = game.turn;
            lobbyGameList[_i].turn.isYours = game.currentPlayer === myPlayerId;
            lobbyGameList[_i].opponentName = game.getEnemyOfPlayerId(myPlayerId).name;
...
function renderChallengeList() {
  // Create the DOM.
  var $newList = $('<ul class="challenge-list">');

  var _loop = function _loop(i) {
    var $challengeItem = $('<li>').text(pendingChallenges[i].challengerName).data('userId', pendingChallenges[i].challengerId).on('click', function () {
      var title = 'You have been challenged by ' + pendingChallenges[i].challengerName;
      var message = 'How do you respond to this challenge?';
      showAcceptDeclineCancel(title, message).then(function (button) {
  ...
  for (var i = 0; i < pendingChallenges.length; i++) {
    _loop(i);
  }
...
function renderLobbyGameList() {
  var $gameList = $('<ul class="game-list">');

  for (var i = 0; i < lobbyGameList.length; i++) {
    var game = lobbyGameList[i];

    var $gameItem = $('<li class="my-game">').data('gameId', game.gameId);
    var $opponent = $('<div class="opponent">').text('vs ' + game.opponentName);
    var $bottom = $('<div class="bottom">');
    var $turn = $('<div class="turn">').text('Turn ' + game.turn.number);
...
function renderGameBoard() {
  ...
  if (currentGame.currentPlayer === player.pid && currentGame.currentRoll) {
    ...
    for (var _i2 = 0; _i2 <= 14; _i2++) {
      if (validMoves[_i2] === true) {
        ...
        var arrowPath = Arrow.prototype.getArrowPath(_i2, currentGame.currentRoll, 'player');
        var arrow = new Arrow(arrowPath);

        var svgAttributes = {
          x: (tToX(_i2, 'p') + arrow.offset.x) * 100,
          y: (tToY(_i2, 'p') + arrow.offset.y) * 100
        };

        $('svg.cell.t-' + _i2 + '.l-player').addClass('valid').after(arrow.svg.attr(svgAttributes).addClass('player'));
      }
    }
  }
  ...
  for (var _i5 = 0; _i5 < pids.length; _i5++) {
    var _lastMoves = currentGame.lastMoves[pids[_i5]];
    var playerTrack = pids[_i5] === myPlayerId ? 'player' : 'enemy';
...
function renderGameInformation(player, enemy) {
  ...
  // Update the dice box.
  var $rollBox = $('.stats .dice').removeClass('go');
  var $rollContent = $('.stats .dice .content');
  ...
  var playerTokensDone = player.tokensDone;
  var enemyTokensDone = enemy.tokensDone;
  var $playerTokenContainer = $('<div class="pieces">');
  var $enemyTokenContainer = $('<div class="pieces">');

  for (var i = 0; i < 7; i++) {
    var $playerToken = $('<div class="token">');
    var $enemyToken = $('<div class="token">');
...
function openPlayerMenu() {
  ...
  var playerName = $(this).data('userName');
  ...
  var $closeButton = $('<div class="close">').text('Close');

  if (playerId === myPlayerId) {
    // Show different options if the player clicks on them self.
    var $logoutButton = $('<li class="logout">').text('Logout');
    $optionsMenu.append($logoutButton);
  } else {
    // Show interactions with other players.
    var $challengeButton = $('<li class="challenge">').text('Challenge');
  ...
  $overlay.find('.challenge').off('click').on('click', function () {
    var title = 'Challenge ' + playerName;
    var message = 'Send a challenge request to ' + playerName + '?';
    showOkCancel(title, message).then(function (button) {
  ...
  $overlay.find('.logout').off('click').on('click', function () {
    var title = 'Logout?';
    var message = 'Are you sure you wish to logout?';
    showOkCancel(title, message).then(function (button) {
...
function showMessageBox(title, message) {
  ...
  var $okButton = $('<button>').text('OK').on('click', dismissOverlay);
  var $controls = $('<div class="controls">');
  $controls.append($okButton);

  var $title = $('<div class="heading">').text(title);
...
Arrow.prototype.fromString = function (string) {
  ...
  for (var _i6 = 0; _i6 < this.arrowDetails.length; _i6++) {
    this.arrowDetails[_i6].x -= this.offset.x;
    this.arrowDetails[_i6].y -= this.offset.y;
  }
...
function moveToken(gameId, pid, t, m) {
  return new Promise(function (resolve, reject) {
    ...
    var lane = pid === myPlayerId ? 'p' : 'e';
    var laneFull = pid === myPlayerId ? 'player' : 'enemy';
    var otherLane = pid === myPlayerId ? 'enemy' : 'player';
    var d = t + m;
    ...
    // Animate the token.
    animateToken(t, m, lane).then(function () {
      ...
      if (d < 15) {
        // Place the new token.
        $svgBoard.find('svg.cell.t-' + d + '.l-' + laneFull).addClass(laneFull)
...
window.animationTest = function (trackIndex, movementPoints, laneName) {
  ...
  var pathCoordinates = getPathCoords(trackIndex, movementPoints, laneName);
  var initial = pathCoordinates[0];

  var moveTime = 1000;
  var animTime = 900;

  var $token = $('circle.token.ani-token');
  if (!$token.length) {
    $token = $(svgEl('circle')).attr({
      cx: 0,
      cy: 0,
      r: 22
    }).addClass('token').addClass('ani-token');
  }
  ...
  .css('transition', 'transform ' + animTime + 'ms ease').appendTo($svgBoard);

  var _loop2 = function _loop2(i) {
    var current = pathCoordinates[i];
    var previous = pathCoordinates[i - 1];

    var sleepDelay = (i - 1) * moveTime;
    var sleepGlobalDelay = 10;
    var sleepInsert = i * 10;
    var sleepTotal = sleepGlobalDelay + sleepDelay + sleepInsert;
...
var Player = __webpack_require__(132);
...
function Game(pid1, pid2) {
   ...
   *
   * @type {{message: string, turn: int}[]} The messages for this game.
   */
  this.messages = [];
 ...
 * @return {{message: string, turn: int}} The message data.
 */
